...
pub fn threegui(ui: &mut egui::Ui, size: impl Into<Vec2>, user_func: impl FnMut(&mut ThreeUi) + Sized) {
    egui::Frame::canvas(ui.style()).show(ui, |ui| {
        ThreeWidget::new(size, ui.next_auto_id()).show(ui, user_func)
    });
}
...
impl ThreeWidget {
    pub fn new(size: impl Into<egui::Vec2>, id: impl Into<egui::Id>) -> Self {
        Self {
            desired_size: size.into(),
            id: id.into(),
        }
    }
